...
自宅LAN内に設置したAtomCam 3台の映像を **1画面に集約** して閲覧できるセキュリティモニターの仕組みを構築します。
...
> RTSP配信はAtomCam内のmicroSDに導入したatomcam_toolsによって有効化されます。
...
本システムの主機能です。AtomCam 3台の映像をブラウザの1画面に集約し、セキュリティモニターとして使用します。

### 5.1 atomcam_tools の役割

AtomCamはデフォルトでRTSP配信機能を持ちません。microSDにatomcam_toolsをインストールすることで以下が有効になります。
...
AtomCam 3台からRTSPでストリームをプルし、WHEPでブラウザに配信します。
...
> **注意**: MediaMTX v1.18.1では `sourceReconnectPause` は廃止されています。削除してください。
...
ChromeブラウザはmDNS obfuscationによりローカルIPアドレスをICE候補として公開しません。  
coturnをSTUNサーバーとして使用することで、LAN内のICE候補を正しく解決しWebRTC接続を確立します。
...
WebUIはrpi4-1のnginxから配信されます。vostroのChromeキオスクモードでの表示に最適化されています。
...
vostro（1280×800）のキオスクモードでスクロールなしに3カメラを表示します。
...
> グリッドセルからの映像はみ出しを防止します（CSS flex/grid の `min-height: auto` 問題への対処）。

#### 映像stall検出（startStallDetector）

映像に焼き込まれたタイムスタンプはJavaScriptから読み取れないため、  
`video.currentTime` の停止を監視することで映像フリーズを検出します。
...
`position: fixed` のCSSオーバーレイで全画面表示を実現します。
...
vostroは専用表示端末としてChromeをキオスクモードで常時起動します。
...
ログイン時(現在、ログイン認証なし)に自動起動することで、電源投入後に自動的に監視画面が表示されます。
...
macmini2010はrsyncで受信した録画ファイルの保持期間管理と、ログファイルの削除を行います。
...
macmini2010は常時起動せず1日数時間のみ稼働します。  
起動後の初回rsyncは数時間〜十数時間分の差分を一括処理しますが、これは設計上の想定動作です。  
macmini2010が停止中はrpi4-0のrsyncが失敗しますが、次回起動時に差分が吸収されます。
...
当初、vostro(Debian13)上に、go2rtc、cheome(ブラウザでの表示)機能を集約したため、CPU温度上昇が非常に大きく、機能を他ノードへ移行した経緯から、CPU使用率、CPU温度、ディスクIO、メモリ使用率については、メトリクスを収集し、グラフ表示を行うことにします。
