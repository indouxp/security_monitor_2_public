#!/bin/bash
# TC-030: collect_mem が MemTotal/MemAvailable から使用率と総メモリ量を算出する
...
tc_main() {
    ...
    run_script
    check_eq "メモリ使用率が25.0である" "$(json_field mem)" "25.0"
    check_eq "総メモリ量が8000000である" "$(json_field mem_total)" "8000000"
}
