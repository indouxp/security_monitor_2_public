#!/bin/bash
################################################################################
# 概要        : backup.sh 単体テストの初期化スクリプト
#               試験対象の存在・構文、expect の有無を確認し、
#               フェイクコマンドへ実行権限を付与して作業ディレクトリを作り直す。
#               backup.sh は expect で擬似端末を供給するため無改修で試験する。
# Created     : 2026-05-22
# Author      : Tsystem
################################################################################
set -eEuo pipefail
...
# ---- 設定 ----
readonly SRC="${MY_DIR}/../../inventory/backup.sh"   # 試験対象
readonly BIN="${MY_DIR}/bin"                     # フェイク scp の配置先
