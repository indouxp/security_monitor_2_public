#!/bin/bash
################################################################################
# 概要        : push_metrics.sh 単体テストの初期化スクリプト
#               試験対象の存在・構文、python3 の有無を確認し、
#               作業ディレクトリを作り直す。
#               push_metrics.sh は取得元パスが環境変数化済みのため、
...
MY_DIR=$(cd "$(dirname "$0")" && pwd)            # ut/push_metrics.sh/
readonly MY_DIR
readonly LOG_PATH="${MY_DIR}/init.sh.log"        # ログ（毎回上書き＝直近のみ保持）
exec > >(tee "$LOG_PATH") 2>&1                   # 全出力をログと画面の両方へ

# ---- 設定 ----
readonly SRC="${MY_DIR}/../../inventory/client/lin/push_metrics.sh"  # 試験対象
...
# モック受信サーバーに必要な python3 の存在確認
command -v python3 >/dev/null || { log "ERROR: python3 が見つかりません"; exit 1; }
log "python3 確認OK: $(command -v python3)"
